...
class _BuySellPageState extends State<BuySellPage> with SingleTickerProviderStateMixin {
  ...
  final List<String> _categories = ['All', 'Sedan', 'SUV', 'Hatchback', 'Truck', 'Motorcycle'];
  final List<String> _priceRanges = ['All', 'Under 10L', '10L-20L', '20L-50L', 'Above 50L'];
  ...
  List<Map<String, dynamic>> get _filteredVehicles {
    return _vehicles.where((vehicle) {
      bool categoryMatch = _selectedCategory == 'All' || vehicle['category'] == _selectedCategory;
      bool priceMatch = _selectedPriceRange == 'All' || _isPriceInRange(vehicle['price'], _selectedPriceRange);
      return categoryMatch && priceMatch;
    }).toList();
  }

  bool _isPriceInRange(String price, String range) {
    int priceValue = int.parse(price.replaceAll(',', '').replaceAll('Rs. ', ''));
    switch (range) {
      case 'Under 10L':
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Buy & Sell Vehicles', style: TextStyle(color: Colors.white)),
        ...
        children: [
          ...
          Expanded(
            child: TabBarView(
              ...
              children: [
                // For Sale Tab
                _buildVehicleList(_filteredVehicles.where((v) => v['type'] == 'sell').toList()),
                // Looking to Buy Tab
                _buildVehicleList(_filteredVehicles.where((v) => v['type'] == 'buy').toList()),
              ],
            ),
          ),
        ],
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: isBuyRequest ? Colors.orange : Colors.green,
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Messaging ${vehicle['seller']}')),
                      );
                    },
